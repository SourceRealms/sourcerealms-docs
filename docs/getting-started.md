...
Not sure where to start? Run **`/help`** any time for a quick overview of Nations, Skills,
Leaderboards, RTP, Voting, and the Server Store, all in one menu.
...
1. **Decide: found your own nation, or join an existing one?**
     ...
     - Joining: ask an existing nation's leader/trusted for `/nation invite <you>`, then run
       `/nation accept`.
2. **Claim some land**: stand in a chunk and run `/nation claim` (needs the *Claim Land*
   permission - Trusted has it by default). See [Claims & Territory](nations/claims-territory.md).
3. **Set your capital**: `/nation capital set <name>` gives your nation a named home point everyone
...
ideology, capital, bank, claims, members.
